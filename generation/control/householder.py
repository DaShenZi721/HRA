...
def _find_modules_old(
    ...
    search_class: List[Type[nn.Module]] = [nn.Linear],
    exclude_children_of: Optional[List[Type[nn.Module]]] = [HouseholderInjectedLinear],
):
    ret = []
...
def inject_trainable_householder(
    model: nn.Module,
    ...
    for _module, name, _child_module in _find_modules(
        ...
        _tmp = HouseholderInjectedLinear(
...
def inject_trainable_householder_with_norm(
    model: nn.Module,
    target_replace_module: Set[str] = DEFAULT_TARGET_REPLACE,
    verbose: bool = False,
    r: int = 4,
...
def inject_trainable_householder_extended(
    model: nn.Module,
...
def inject_trainable_householder_conv(
    model: nn.Module,
