...
class HRALayer(nn.Module, LycorisLayer):
    ...
    def create_adapter_parameters(self, adapter_name: str, r: int, shape: Tuple[int, ...], block_share: bool):
        ...
        self.hra_r[adapter_name] = nn.Parameter(weight.new_zeros(shape[0], r))

    def reset_adapter_parameters(self, adapter_name: str):
        nn.init.kaiming_uniform_(self.hra_r[adapter_name], a=1 / self.eps[adapter_name])
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        Args:
            ...
            chra (`bool`): Whether to use the constrained variant of HRA or not.
            eps (`float`):
                The control strength of CHRA. The freedom of rotation. Only has an effect if `chra` is set to True.
            block_share (`bool`): Whether to share the HRA parameters between blocks or not.
        """
        if r <= 0:
    ...
    def get_delta_weight(self, adapter_name: str) -> torch.Tensor:
        hrft_v = self.hra_r[adapter_name]
        in_features = self.hra_r[adapter_name].size(0)
        device = self.hra_r[adapter_name].device
        dtype = self.hra_r[adapter_name].dtype

        unit_v_list = [hrft_v[:, i].view(-1,1) / (torch.sqrt(torch.sum(hrft_v[:,i] ** 2) + self.eps[adapter_name])) for i in range(self.r[adapter_name])]
    ...
    def _block_diagonal(self, hra_r: torch.Tensor, rank: int) -> torch.Tensor:
        if hra_r.shape[0] == 1:
            # block share
            blocks = [hra_r[0, ...] for i in range(rank)]
        else:
            blocks = [hra_r[i, ...] for i in range(rank)]
    ...
    def _project_batch(self, hra_r, eps=1e-5):
        # scaling factor for each of the smaller block matrix
        eps = eps * 1 / torch.sqrt(torch.tensor(hra_r.shape[0]))
        I = (  # noqa: E741
            torch.zeros((hra_r.size(1), hra_r.size(1)), device=hra_r.device, dtype=hra_r.dtype)
            .unsqueeze(0)
            .expand_as(hra_r)
        )
        diff = hra_r - I
        norm_diff = torch.norm(hra_r - I, dim=(1, 2), keepdim=True)
        mask = (norm_diff <= eps).bool()
        out = torch.where(mask, hra_r, I + eps * (diff / norm_diff))
        return out
